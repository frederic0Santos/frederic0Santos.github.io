...
function configurarFiltros(produtos) {
    ...
    document.querySelector("#ordenarRating").addEventListener("change", function () {
        filtrarEAtualizarProdutos(produtos);
    });
...
function filtrarEAtualizarProdutos(produtos) {
    ...
    const ordenacao = document.querySelector("#ordenarRating").value;
...
function removeALL(){
    ...
    cesto.querySelectorAll("article").forEach(article => cesto.removeChild(article));
    salvarCesto();
    atualizarTotal();
    atualizarBotaoComprar();
}
...
function atualizarBotaoComprar() {
    ...
    const existingButton2 = comprarSection.querySelector(".botao-remover");{
        if(existingButton2) {
        comprarSection.removeChild(existingButton2);
        }
    }
    ...
    if (cesto.length > 0) {
        ...
        botaoComprar.addEventListener("click", function () {
            ...
            .then(dados => {
                ...
                mensagem.textContent = `Mensagem: ${dados.message}`;
                comprarSection.appendChild(valorApagar);
                comprarSection.appendChild(referencia);
                comprarSection.appendChild(mensagem); 
            })
            .catch(error => {
        ...
        comprarSection.appendChild(botaoRemover);
    }
